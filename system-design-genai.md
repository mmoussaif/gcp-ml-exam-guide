...
| **IVF**              | Inverted File Index. Clustering-based ANN. Clusters vectors into groups (e.g., 100 clusters), then only searches relevant clusters at query time.                                                     | Uses less memory than HNSW. Good when index size is a constraint. Lower recall than HNSW; requires tuning nprobe (how many clusters to search per query). |
...
| **CIDEr**                  | Consensus-based Image Description Evaluation. Caption metric that compares a generated caption to multiple human reference captions (e.g. 3–5 per image). Uses TF-IDF–weighted n-grams; rewards captions that match what most references mention. Higher = better. | Standard metric for image captioning. Best correlation with human judgment. Datasets have several captions per image; CIDEr rewards consensus, not just one reference.       |
